...
public class ChordProtocols {
	...
	public void fixFingerTable() {
		...
		try {
			...
			BigInteger hash = Hash.addressSize();
			...
			for(int i = 0; i < numberOfBits; i++) {
				...
				BigInteger succnodeId = chordnode.getNodeID().add(k);
				succnodeId = succnodeId.mod(hash);
				
				NodeInterface succnode = chordnode.findSuccessor(succnodeId);
